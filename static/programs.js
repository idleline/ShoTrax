...
function renderProgramList() {
    ...
    list.innerHTML = programState.programs.map(program => `
        <button
            class="program-list-item ${program.id === programState.activeProgramId ? "active" : ""}"
            type="button"
        ...
        <article class="program-task ${task.completed ? "is-complete" : ""}">
            ...
            <div class="program-task-content">
                <div class="d-flex flex-wrap align-items-start justify-content-between gap-2">
                    <div>
                        <h4 class="program-task-title">${escapeHtml(task.title)}</h4>
                        ${task.description ? `<p class="program-task-description">${escapeHtml(task.description).replaceAll("\n", "<br>")}</p>` : ""}
                    </div>
                    <span class="star-reward ${task.completed ? "earned" : ""}">+${task.reward_stars} ★</span>
                </div>
...
function renderCategories(program, openCategoryIds = new Set()) {
    ...
    container.innerHTML = `
        <div class="d-flex align-items-end justify-content-between gap-3 mb-3">
            ...
            <span class="small text-muted">Expand a category to update progress</span>
        </div>
                ...
                >
                    ...
                    <div class="program-category-body">
                        ...
                        ${category.tasks.length
                            ? category.tasks.map(taskMarkup).join("")
